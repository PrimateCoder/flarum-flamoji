...
- **Faster first picker open, especially in sticker mode.** The emoji library, dataset, and your custom emoji now load in parallel. In sticker mode the picker skips downloading the ~460 KB Unicode emoji dataset entirely (it only needs your custom stickers), so it opens noticeably quicker. Also contributed by @huoxin233.
