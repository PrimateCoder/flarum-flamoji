...
await runSpec({
  ...
  try {
    ...
    // The rename issues ONE bulk request (POST /api/pianotell/rename-
    // category). Fail it: the required outcome is an error alert and
    // zero renames (a single server-side atomic operation).
    let renameCount = 0;
    await page.route('**/api/pianotell/**', (route) => {
      const req = route.request();
      if (/\/api\/pianotell\/rename-category/.test(new URL(req.url()).pathname)) {
        renameCount += 1;
        if (renameCount === 1) {
    ...
    await page.unroute('**/api/pianotell/**');
