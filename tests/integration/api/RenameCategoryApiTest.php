 ...
 * and #2): the admin UI currently renames categories by saving every
 ...
 * fix should provide.
 *
 * ⚠️ EXPECTED TO FAIL (404) until `POST /api/pianotell/rename-category`
 * exists.
 */
...
class RenameCategoryApiTest extends TestCase
{
    ...
    {
        return $this->send($this->request('POST', '/api/pianotell/rename-category', [
            'authenticatedAs' => $as,
    ...
    public function rename_endpoint_requires_authentication(): void
    {
        ...
        $response = $this->send($this->request('POST', '/api/pianotell/rename-category', [
